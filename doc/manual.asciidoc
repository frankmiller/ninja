...
`%B`:: The build rule being started.
`%b`:: The build rule being completed.
`%d`:: The duration of the completed edge or `---` for a starting edge.
...
The default progress status is `"[%s/%t] %B"` Another example of possible progress status
could be `"[%u/%r/%f] %B"`. Only one of %B or %b may be provided. If neither is
provided, %B is automatically added at the end of the status.
